...
OUTPUT FORMAT — JSON THUẦN, KHÔNG THÊM BẤT CỨ TEXT NÀO BÊN NGOÀI:
{
  ...
  ${isFemaleClothing ? `"imagePrompts": [
    ...
    "CONCEPT 5 — Sinh hoạt đời thường miền Tây. Write a complete English image prompt: [CHARACTER] ${femaleCharacterSpec} Fully absorbed in what she is doing, not posing. [OUTFIT] She is wearing [PRODUCT INFO — copy only what is explicitly stated here, do NOT invent or add any color, fabric, or style not mentioned: \\"${product.description ?? product.name}\\". Describe the garment accurately using only these details.] as casual home wear, looking completely natural and effortless. [ACTIVITY] Choose one authentic rural Vietnamese daily scene that fits the outfit best: (a) squatting picking and sorting fresh vegetables in a wicker basket on a simple tiled floor, (b) standing at a rustic outdoor washbasin splashing water on her face with both hands, eyes closed, or (c) bending slightly to scoop water from a large traditional clay water jar (lu) using a wooden or coconut-shell ladle. [SETTING] Rural Mekong Delta Vietnamese home — simple earthen or tiled floor, weathered wooden walls, clay water jar, wicker baskets, tropical plants nearby, dappled natural light. Warm soft morning light. [TECHNICAL] ${technicalSpec}"
