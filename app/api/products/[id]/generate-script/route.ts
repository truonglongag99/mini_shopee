...
OUTPUT FORMAT — JSON THUẦN, KHÔNG THÊM BẤT CỨ TEXT NÀO BÊN NGOÀI:
{
  ...
  ${isFemaleClothing ? `"imagePrompts": [
    ...
    "CONCEPT 4 — POV nữ. Write a complete English image prompt: [SHOT TYPE] First-person POV shot looking down — camera positioned at chest height, viewer IS the woman wearing the outfit. [VISIBLE ELEMENTS] The clothing covering the body from chest to legs or feet. Two hands with natural feminine appearance — slightly chubby soft arms and hands, no heavy jewelry, light nail color or bare nails — resting casually, one on hip, one by side or in pocket. [OUTFIT] She is wearing [PRODUCT INFO — copy only what is explicitly stated here, do NOT invent or add any color, fabric, or style not mentioned: \\"${product.description ?? product.name}\\". Describe the garment accurately using only these details.]. [SETTING] ${outdoorScene4}. Warm soft natural light falling on the outfit. Ground or background naturally visible below. [TECHNICAL] ${technicalSpec} No face visible.",
